...
卡片展示框统一为 `4:5`，只影响展示裁切，不改变封面源文件。新导入 Skill 如果没有可追溯样例，使用 `coverStatus: "needs_sample"`，必须先运行并保存一次结果后才能写入封面。第三方 Skill 的作者、来源与许可见 [`THIRD_PARTY_NOTICES.md`](../THIRD_PARTY_NOTICES.md)。
...
The card frame is fixed at `4:5`; this affects presentation cropping only and does not modify the cover source file. A newly imported Skill without a traceable sample uses `coverStatus: "needs_sample"` and must produce and save one result before a cover can be assigned. See [`THIRD_PARTY_NOTICES.md`](../THIRD_PARTY_NOTICES.md) for authors, sources, and licenses.
